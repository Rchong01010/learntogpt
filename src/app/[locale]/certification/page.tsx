...
import { ArrowRight, Award, CheckCircle, Shield, Star } from "lucide-react";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "certification.meta" });
  const baseUrl =
    process.env.NEXT_PUBLIC_APP_URL || "https://learntogpt.com";
  const pathForLocale = (loc: string) =>
    `${baseUrl}${loc === routing.defaultLocale ? "" : `/${loc}`}/certification`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: "Learn to GPT — ChatGPT Certification",
        },
      ],
    },
    twitter: {
            ...
            "@graph": [
              {
                ...
                url: `${baseUrl}${locale === routing.defaultLocale ? "" : `/${locale}`}/certification`,
                inLanguage: locale,
                isPartOf: {
                  "@type": "WebSite",
                  name: "Learn to GPT",
              ...
              },
              {
                ...
                itemListElement: [
                  ...
                  },
                  {
                    ...
                    position: 2,
                    name: t("heading"),
                    item: `${baseUrl}${locale === routing.defaultLocale ? "" : `/${locale}`}/certification`,
                  },
                ],
              },
            ],
        ...
        <section className="px-6 py-16">
          <div className="mx-auto max-w-[900px]">
            ...
            <div className="mx-auto mt-10 grid max-w-[900px] gap-6 md:grid-cols-3">
              {/* Beginner */}
              <div className="relative rounded-[24px] border-[4px] border-ink bg-cream p-[32px_24px_28px] shadow-[4px_4px_0px_#1c1917]">
                <div className="mb-4 flex size-[56px] items-center justify-center rounded-full border-[3px] border-ink bg-[#d0f0ea] shadow-[2px_2px_0px_#1c1917]">
                  <Star className="size-6 text-teal" />
                </div>
                ...
                <ul className="space-y-2">
                  {[1, 2, 3].map((j) => (
                    <li
                      key={j}
                      className="flex items-start gap-2 text-[0.85rem] text-text-secondary"
                    >
                      <CheckCircle className="mt-0.5 size-4 shrink-0 text-teal" />
                      {t(`levels.beginner.skill${j}`)}
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="relative rounded-[24px] border-[4px] border-ink bg-cream p-[32px_24px_28px] shadow-[4px_4px_0px_#1c1917]">
                ...
                <div className="mb-2 text-[1.2rem] font-bold text-ink">
                  {t("levels.practitioner.title")}
                </div>
                <p className="mb-4 text-[0.9rem] leading-[1.6] text-text-secondary">
                  {t("levels.practitioner.description")}
                </p>
                <ul className="space-y-2">
                  {[1, 2, 3].map((j) => (
                    <li
                      ...
                      className="flex items-start gap-2 text-[0.85rem] text-text-secondary"
                    >
                      <CheckCircle className="mt-0.5 size-4 shrink-0 text-orange" />
                      {t(`levels.practitioner.skill${j}`)}
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="relative rounded-[24px] border-[4px] border-ink bg-cream p-[32px_24px_28px] shadow-[4px_4px_0px_#1c1917]">
                <div className="mb-4 flex size-[56px] items-center justify-center rounded-full border-[3px] border-ink bg-[#e8e0f0] shadow-[2px_2px_0px_#1c1917]">
                  <Shield className="size-6 text-[#7c3aed]" />
                </div>
                <div className="mb-2 text-[1.2rem] font-bold text-ink">
                  {t("levels.architect.title")}
                </div>
                <p className="mb-4 text-[0.9rem] leading-[1.6] text-text-secondary">
                  {t("levels.architect.description")}
                </p>
                <ul className="space-y-2">
                  {[1, 2, 3].map((j) => (
                    <li
                      ...
                      className="flex items-start gap-2 text-[0.85rem] text-text-secondary"
                    >
                      <CheckCircle className="mt-0.5 size-4 shrink-0 text-[#7c3aed]" />
                      {t(`levels.architect.skill${j}`)}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="mx-auto max-w-[800px]">
            ...
            <div className="mt-10 space-y-6">
              {[1, 2, 3].map((i) => (
                ...
                >
                  <div className="mb-1 text-[1.05rem] font-bold text-ink">
                    {t(`proves.point${i}.title`)}
                  </div>
                  <p className="text-[0.9rem] leading-[1.6] text-text-secondary">
                    {t(`proves.point${i}.body`)}
                  </p>
                </div>
              ))}
            </div>
