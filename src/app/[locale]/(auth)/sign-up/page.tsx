...
export default function SignUpPage() {
  ...
  // Store plan intent in a cookie when ?plan=pro is present
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get("plan") === "pro") {
      document.cookie = `plan_intent=pro; path=/; max-age=300; SameSite=Lax; Secure`;
    }
  }, []);
  ...
  async function handleGoogleSignUp() {
    setError(null);
    // Google OAuth has its own bot protection, no captcha needed
    const supabase = createSupabaseBrowser();
